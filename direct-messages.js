    ...
    if (event.httpMethod === 'POST') {
      ...
      if (isAdmin) {
        ...
        if (wantAsAdmin && (meRole === 'owner' || meRole === 'admin')) {
          ...
          // 🔴 需求：owner 主动发给同学 → recipientType=owner（对方在「👑 私信站主」看到）；admin 主动发给同学 → recipientType=admin（对方在「🛡️ 私信管理员」看到）
          // 1) 先以发送者角色决定默认通道（如果 body 没有强指定）
          if (!(body.recipientType && VALID_RECIPIENT_TYPES.includes(String(body.recipientType)))) {
            recipientType = (meRole === 'owner') ? 'owner' : 'admin';
          }
          // 安全检查：role=admin 仍然禁止发 recipientType=owner 的给别人（站主类私信屏蔽）
          if (meRole === 'admin' && recipientType === 'owner') {
            return { statusCode: 403, headers, body: JSON.stringify({ error: '管理员无权代发站主类私信（recipientType=owner），请使用「私信管理员」通道。' }) };
          }
          sender = 'admin'; // 以管理员/站主视角发给目标同学（recipientType=owner 就是"站主身份"发，=admin 就是"管理员身份"发，对方在对应 Tab 收到）
          studentId = targetSid;
          studentName = targetS.nickname || targetSid;
          // 2) 如果 body 明确指定了 recipientType，优先 body（但已在上面安全检查）
          if (body.recipientType && VALID_RECIPIENT_TYPES.includes(String(body.recipientType))) {
            recipientType = String(body.recipientType);
          }
        } else {
